...
(function (factory) {
  ...
  var Page = function (element, options) {
    var defaultOption = {
      ...
      firstBtnText: null,
      lastBtnText: null,
      prevBtnText: '<i class="fa fa-step-backward" aria-hidden="true"></i>',
      nextBtnText: '<i class="fa fa-step-forward" aria-hidden="true"></i>',
      loadFirstPage: true,
      remote: {
        url: null,
        params: null,
      ...
      pageElementSort: ['$jump', '$page', '$size', '$info'],
      showInfo: false,
      infoFormat: '{start} ~ {end} of {total} records',
      noInfoText: '0 records',
      showJump: false,
      jumpBtnText: 'Go',
      showPageSizes: false,
  ...
  Page.prototype = {
    ...
    initHtml: function () {
      //init jump module
      var jumpHtml = '<div class="m-pagination-group"><input data-page-btn="jump" type="text"><button data-page-btn="jump" type="button" class="pagerGo">' + this.options.jumpBtnText + '</button></div>';
      ...
      this.$element
        .on('click', {page: this}, function (event) {
          if ($(event.target).is('button')) {
            ...
            else if ($(event.target.parentElement).attr('data-page-index') !== undefined) {
    ...
    jumpEventHandler: function (inputValue, event) {
      if (!inputValue) {
        this.$jump.find('input').removeClass('error');
      } else if (!pagination.check.checkJumpPage(inputValue)) {
        this.$jump.find('input').addClass('error');
    ...
    renderPagination: function () {
      ...
      this.$info.html(
        pagination.core.renderInfo(this.currentPageIndex, this.currentPageSize, this.total)
      );
  ...
  pagination.core = {
    ...
    */
    ...
    renderInfo: function (currentPageIndex, currentPageSize, total) {
      if (total <= 0) {
        return '<div type="pagerNoRecord">';
      } else {
        ...
        return `
              <div style="display: flex">
                  <div type="pageFromLabel">from</div>
                  <div type="pagerStartNum">${startNum}</div>
                  <div type="pageToLabel">To</div>
                  <div type="pagerEndNum">${endNum}</div>
                  <div type="pagerSpace">-</div>
                  <div type="pagerTotal">${total}</div>
                  <div type="pageTotalLabel">Total</div>
              </div>
               `;
      }
    },
